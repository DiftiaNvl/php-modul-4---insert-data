...
if($_POST){
    ...
    if(empty($id_pelanggan)){
        ...
        $insert=mysqli_query($conn,"INSERT INTO pelanggan(id_pelangga, nama, alamat, telp) VALUES ('".$id_pelanggan."','".$nama_pelanggan."', '".$alamat_pelanggan."', '".$notelp_pelanggan."')");

        if($insert){

            echo "<script>alert('Sukses membuat akun');location.href='tambah_pelanggan.php';</script>";

        } else {

            echo "<script>alert('Gagal membuat akun');location.href='tambah_pelanggan.php';</script>";
